...
```plaintext (gdb) set print pretty on ```
...
```plaintext (gdb) add-auto-load-safe-path [/path/to/executable] ```
...
```plaintext (gdb) source [/path/to/boost]/libs/unordered/extra/boost_unordered_printers.py ```
